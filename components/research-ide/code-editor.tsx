...
import { Play, Save, Download, Upload, Terminal, FileText, Database, Brain } from "lucide-react"
...
export function CodeEditor({
  ...
  const getLanguageBadgeColor = () => {
    switch (language) {
      ...
      case "r": return "bg-purple-500/10 text-purple-400 border-purple-500/20"
      case "javascript": return "bg-yellow-500/10 text-yellow-400 border-yellow-500/20"
      case "sql": return "bg-green-500/10 text-green-400 border-green-500/20"
  ...
  return (
    <div className="flex flex-col h-full bg-card border border-border rounded-lg overflow-hidden">
      {/* Editor Header */}
      <div className="flex items-center justify-between p-3 border-b border-border bg-accent/5">
        <div className="flex items-center gap-3">
          <Badge className={getLanguageBadgeColor()}>
            {language.toUpperCase()}
          </Badge>
          <span className="text-sm text-muted-foreground">
            Research Terminal - Session {sessionId || "default"}
          </span>
        </div>
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <div className="flex-1 flex flex-col">
          <div className="flex items-center gap-2 px-3 py-2 bg-accent/5 border-b border-border text-sm text-muted-foreground">
            <FileText className="w-4 h-4" />
            Code Editor
            <span className="ml-auto text-xs">Ctrl+Enter to execute</span>
          </div>
...
`}`}
        ...
        <div className="w-1/2 border-l border-border flex flex-col">
          ...
          <div
            ref={outputRef}
            className="flex-1 p-4 bg-background/50 font-mono text-sm overflow-auto"
          >
            {output ? (
      ...
      {/* Status Bar */}
      <div className="flex items-center justify-between px-3 py-2 bg-accent/5 border-t border-border text-xs text-muted-foreground">
        ...
        <div className="flex items-center gap-2">
          ...
          <span>Connected to Research Database</span>
          <Brain className="w-3 h-3 ml-2" />
          <span>CROWE LOGIC Assistant Ready</span>
        </div>
      </div>
    </div>
  )
